...
def validate(model, data_loader_valid, loss_func, device, ds_size):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (image, target) in enumerate(data_loader_valid):
            ...
            mask[target!=255] = 1   # Ignore pixels with value 255
            metrics = torchtrainer.perf_funcs.segmentation_accuracy(output, target, mask=mask)
...
def train(model, ds_train, ds_valid, experiment_folder, loss, class_weights, epochs, lr, batch_size_train, batch_size_valid, momentum=0.9, weight_decay=0., scheduler_power=0.9, 
    ...
    if loss=='cross_entropy':
        loss_func = nn.CrossEntropyLoss(weight=torch.tensor(class_weights, device=device), ignore_index=255) 
...
def run(user_params):
    ...
    if params['model_type']=='unet':
        model = torchtrainer.models.resunet.ResUNet(params['model_layers'], params['model_channels'], in_channels=3)
    elif params['model_type']=='resnetseg':
        model = torchtrainer.models.resnet_seg.ResNetSeg(params['model_layers'], params['model_channels'], in_channels=3)
